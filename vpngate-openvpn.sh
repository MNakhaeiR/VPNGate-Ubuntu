...
quick_connect() {
    ...
    local config_url="https://www.vpngate.net/common/openvpn_download.aspx?sid=1&udp=1"
    wget -q -O "$CONFIG_DIR/quick.ovpn" "$config_url"
    
    if [ -s "$CONFIG_DIR/quick.ovpn" ]; then
        log "Connecting..."
        sudo openvpn --config "$CONFIG_DIR/quick.ovpn" --daemon --writepid /var/run/openvpn-vpngate.pid
        sleep 3
        status
    else
...
case "$1" in
    ...
    *)
        ...
        echo "Usage: $0 {connect|quick|disconnect|status}"
        echo ""
        echo "  connect     - Find and connect to best server"
        echo "  quick       - Quick connect to any available server"
